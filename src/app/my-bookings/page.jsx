...
import { MdBook, MdCalendarToday, MdAccessTime, MdAttachMoney, MdCancel, MdHourglassEmpty, MdCheckCircle, MdRemoveCircle } from "react-icons/md";
...
export default function MyBookingsPage() {
    ...
    const fetchMyBookings = async () => {
        ...
        try {
            ...
            const res = await fetch(`${apiUrl}/booking?email=${user.email}`, { cache: 'no-store' });
    ...
    const getStatusIcon = (status) => {
        switch (status) {
            ...
            case "pending":
            default:
                return <MdHourglassEmpty className="text-lg animate-spin-slow" />;
        }
    };
                ...
                {bookings.length === 0 ? (
                    <div className="bg-white rounded-3xl p-12 text-center border border-gray-100 shadow-sm max-w-lg mx-auto">
                        ...
                        <p className="text-gray-500 mt-2 mb-6">
                            You haven't booked any arenas yet. Explore facilities to find one!
                        </p>
                        <button
                            ...
                            >
                                ...
                                <div className="space-y-3 flex-grow">
                                    <div className="flex flex-wrap items-center gap-3">
                                        ...
                                        {/* Status Badge */}
                                        <span className={`flex items-center gap-1 text-xs font-bold px-3 py-1 rounded-full border ${getStatusStyle(booking.status)}`}>
                                    ...
                                    <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-gray-500">
                                        <div className="flex items-center gap-1.5">
                                            ...
                                            <span>Date: <span className="font-semibold text-gray-700">{booking.booking_date}</span></span>
                                        </div>
                                        <div className="flex items-center gap-1.5">
                                            <MdAccessTime className="text-orange-500" />
                                            <span>Time: <span className="font-semibold text-gray-700">{booking.time_slot}</span></span>
                                        </div>
                                        <div className="flex items-center gap-1.5">
                                            <MdHourglassEmpty className="text-orange-500" />
                                            <span>Duration: <span className="font-semibold text-gray-700">{booking.hours} hr(s)</span></span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center justify-between md:justify-end gap-6 border-t md:border-t-0 border-gray-100 pt-4 md:pt-0">
                                    ...
                                    {booking.status === "pending" && (
                                        <button 
                                            onClick={() => handleCancelBooking(booking._id, booking.facility_name)}
                                        ...
                                        >
                                            <MdCancel className="text-base" /> Cancel Booking
                                        </button>
                                    )}
                                </div>
                            </div>
